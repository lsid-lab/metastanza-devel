...
const nodeAlphaSym = Symbol("nodeAlpha");
...
export default function (nodesC, edgesC, params) {
  ...
  // const nodeAlphaScale = getScaleFunc(nodeAlphaParams.scale)
  //   .range([nodeAlphaParams.minAlpha, nodeAlphaParams.maxAlpha])
  //   .domain(d3.extent(nodesC, (d) => d[nodeAlphaParams.dataKey]));

  nodesC.forEach((node) => {
    if (node[nodeAlphaParams.dataKey] || node[nodeAlphaParams.dataKey] === 0) {
      node[symbols.nodeAlphaSym] = node[nodeAlphaParams.dataKey];
        // nodeAlphaScale(node[nodeAlphaParams.dataKey]) || nodeAlphaParams.minAlpha;
    } else {
      node[symbols.nodeAlphaSym] = nodeAlphaParams.minAlpha;
    }
  });
